class Restaurant {
    ...
    static fetchRestaurants(apiUrl) {
        return fetch(apiUrl)
            ...
            .then(data => {
                return data.map(restaurant => new Restaurant(restaurant.id, restaurant.nom, restaurant.adresse, restaurant.latitude, restaurant.longitude, restaurant.lienImage));
            })
            .catch(error => {
    ...
    static displayRestaurants(map, restaurants) {
        const customIcon = L.icon({
            iconUrl: '../image/restaurantcarte.png', iconSize: [32, 32], iconAnchor: [16, 16]
        });
